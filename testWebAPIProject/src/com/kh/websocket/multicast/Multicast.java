...
public class Multicast {
	...
	//각 방 별로 사용자들의 정보를 담기위한 Map 객체를 선언하여
	//각 방을 key로 사용자 리스트를 Set<Session>컬렉션 객체를
   ...
   // 해당 방의 사용자 리스트에 갱신을 해주어야 한다.
    ...
    public void onOpen(EndpointConfig config, Session session, @PathParam("chatRoom") String room) {
       ...
       //웹 소켓 세션에 사용자가 별도로 추가하거나 수정할 수 있는 설정 부분을 가져올때 사용하는 메소드
    ...
    public void onMessage(String message, Session session) throws IOException {
      ...
      //같은 방안의 사용자들에게 데이터를 뿌려주는 forEach 메소드 작성
      userList.stream().forEach( x -> {
    	  //람다식 사용부 : 기존 절차지향 언어가 가지는 특성인 익명 함수 구현부분을 객체 지향 언어에서도 흉내내기 위한 소스코드 작성 기술
